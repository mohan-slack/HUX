...
const SpO2Monitor: React.FC = () => {
  const [spo2, setSpO2] = useState<SpO2Data | null>(null);
  const [monitoring, setMonitoring] = useState(false);

  useEffect(() => {
    if (monitoring) {
      SpO2Service.startMonitoring(setSpO2);
      return () => SpO2Service.stopMonitoring();
    } else {
  ...
  return (
    <View style={styles.container}>
      <AppHeader title="SpO₂ Monitor" />
      <AppCard style={styles.card}>
        <View style={styles.valueContainer}>
          {monitoring ? (
            spo2 ? (
              <Text style={styles.value}>{spo2.value}%</Text>
            ) : (
